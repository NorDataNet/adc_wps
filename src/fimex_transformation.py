    ...
    def writeToFile(self):
        ...
        if self.outputType == 'GeoTIFF':
            ...
            if self.interpolateProjString:
                ...
                # Check if grid_mapping variable contains crs_wkt
                grid_mapping_attribute = cdm.getAttribute(variable,'grid_mapping')
                proj_variable = grid_mapping_attribute.getStringValue()
                try:
                    crs_wkt = cdm.getAttribute(proj_variable,'crs_wkt')
                except RuntimeError as e:
                    crs_wkt = None
                    print(e)
            ...
            #print('DEBUG',var_ds)
            #print('DEBUG',var_ds[list(var_ds)[0]].values())
            ...
            if self.interpolateProjString:
                ...
                dst_ds.SetProjection(crs_wkt.getStringValue())

            for i, variable in enumerate(list(var_ds)):
                tmp_attrib_dict = {}
                #print('DEBUG',i,variable,xn,yn)
                ...
                #print('DEBUG',band_attrib)
                #print('DEBUG',variable)
                #print('DEBUG',var_ds[variable].values().shape)
                #print('DEBUG',xn, yn)
                array = np.flipud(var_ds[variable].values().reshape(yn,xn))
                #print('DEBUG',array.shape)
                #print('DEBUG',dst_ds.RasterXSize,dst_ds.RasterYSize)
...
run fimex_transformation.py --input.file=S1B_IW_GRDM_1SDV_20171117T053116_20171117T053148_008317_00EB7D_AFEE.nc --input.type=nc4 --output.file=test.tif --output.type=GTIFF --extract.selectVariables=Amplitude_VH
run fimex_transformation.py --input.file=http://nbstds.met.no/thredds/dodsC/NBS/S2B/2018/02/08/S2B_MSIL1C_20180208T124259_N0206_R095_T28WFU_20180208T131236.nc --input.type=nc4 --output.file=test.tif --output.type=GTIFF --extract.selectVariables=Amplitude_VH

run fimex_transformation.py --input.file=http://thredds.met.no/thredds/dodsC/aromearcticlatest/arome_arctic_pp_2_5km_latest.nc --input.type=nc --extract.selectVariables=wind_speed_of_gust --extract.selectVariables=air_temperature_2m --interpolate.method=nearestneighbor --interpolate.projString="+proj=stere +lat_0=-90 +lon_0=0 +lat_ts=-71 +ellps=WGS84 +datum=WGS84 +units=m" --interpolate.xAxisValues=16207098.2916,16506161.1418,...,46113383.3163 --interpolate.yAxisValues=26623574.2709,27276488.7471,...,91915021.8897 --interpolate.xAxisUnit=m --interpolate.yAxisUnit=m --output.file=aromeArcticTransformed.nc --output.type=nc4

run fimex_transformation.py --input.file=http://thredds.met.no/thredds/dodsC/aromearcticlatest/arome_arctic_pp_2_5km_latest.nc --input.type=nc --extract.reduceToBoundingBox.north=75 --extract.reduceToBoundingBox.south=64 --extract.red
