...
class Process(WPSProcess):
    ...
    def execute(self):
        ...
        #inCommand=["fimex"]
        inCommand=["python3"]
        path_to_transformation_script = "/path/to/adc_wps/src/fimex_transformation.py" #NOTE: specify the filepath to fimex_transformation.py in ../adc_wps/src/
        inCommand.append(path_to_transformation_script)
	...
	sizecom = inCommand[:] #For file size testing
        ...
        sizecom.remove("/home/ubuntu/dev/fimex_transformation_python/fimex_transformation.py")
	    ...
	    try:
		    ...
		    # Aggregated files fix, see https://wiki.met.no/fimex/faq -> "OpENDAP and slicing"
                    ...
                    inCommand.remove(path_to_transformation_script)
                    inCommand.insert(0,'fimex')
		    inCommand.append("--input.config=/path/to/adc_wps/wps/etc/unlimitedTime.ncml")
 		    sizecom.append("--input.config=/path/to/adc_wps/wps/etc/unlimitedTime.ncml")
		    continue
           	self.status.set('{}'.format(err.output,))
            	return "Fimex error: {}".format(err.output,)+"\n From fimex command: {}".format(" ".join(err.cmd),)
            else:
            	self.urlout.setValue("http://IP-ADDRESS"+output_filename[13:]) #NOTE: Add IP address for the server
